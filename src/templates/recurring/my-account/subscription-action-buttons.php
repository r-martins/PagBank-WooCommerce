...
$actions = apply_filters('rm_pagbank_account_recurring_actions', [
    'cancel' => [
        ...
        'url' => wc_get_endpoint_url( 'rm-pagbank-subscriptions', '', wc_get_page_permalink( 'myaccount' ) ) . '?action=cancel&id=' . $subscription->id,
        'class' => 'subscription-button cancel',
    ],
    'suspend' => [
        'name' => __('Suspender Assinatura', RM_PagBank\Connect::DOMAIN),
        'url' => wc_get_endpoint_url( 'rm-pagbank-subscriptions', '', wc_get_page_permalink( 'myaccount' ) ) . '?action=suspend&id=' . $subscription->id,
        'class' => 'subscription-button suspend',
    ],
    'activate' => [
        'name' => __('Ativar Assinatura', RM_PagBank\Connect::DOMAIN),
        'url' => wc_get_endpoint_url( 'rm-pagbank-subscriptions', '', wc_get_page_permalink( 'myaccount' ) ) . '?action=activate&id=' . $subscription->id,
        'class' => 'subscription-button activate',
    ],
    'update' => [
        'name' => __('Atualizar Cartão', RM_PagBank\Connect::DOMAIN),
        'url' => wc_get_endpoint_url( 'rm-pagbank-subscriptions', '', wc_get_page_permalink( 'myaccount' ) ) . '?action=update&id=' . $subscription->id,
